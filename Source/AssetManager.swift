...
open class AssetManager {
  ...
  open static func resolveAssets(_ assets: [PHAsset], size: CGSize = CGSize(width: 720, height: 1280)) -> [UIImage] {
    ...
    for asset in assets {
      imageManager.requestImage(for: asset, targetSize: size, contentMode: .aspectFill, options: requestOptions) { image, info in
				guard let maybeUrl = info?["PHImageFileURLKey"] as? NSURL else { return }
				let stringURL = NSString(string: maybeUrl.absoluteString!)
				print("URL TO FILE >>>>>>>>>>>>> ", maybeUrl)
				NotificationCenter.default.post(name: Notification.Name(rawValue:ImageStack.Notifications.pathToPhoto), object: stringURL)
        if let image = image {
          images.append(image)
        }
      }
